...
void openmp_gemm_opt(int thread_num, float *C, float *A, float *B, uint64_t M, uint64_t N, uint64_t K){
    ...
    // 为每个线程分配buffer
    std::vector<std::vector<float>> A_padding_buffers(real_thread_num);
    std::vector<std::vector<float>> B_padding_buffers(real_thread_num);
    std::vector<std::vector<float>> C_padding_buffers(real_thread_num);

#pragma omp parallel num_threads(real_thread_num) \
            default(none) \
            shared(C, A_padding_buffers, B_padding_buffers, C_padding_buffers) \
                ...
                m_thread, n_thread)
    {
        ...
        size_t a_size = (dim_m_per_thread + m_padding) * K;
        size_t b_size = (dim_n_per_thread + n_padding) * K;
        size_t c_size = (dim_m_per_thread + m_padding) * (dim_n_per_thread + n_padding);

        auto& A_padding_buffer = A_padding_buffers[thread_id];
        auto& B_padding_buffer = B_padding_buffers[thread_id];
        auto& C_padding_buffer = C_padding_buffers[thread_id];

        if (A_padding_buffer.size() < a_size) A_padding_buffer.resize(a_size, 0.0f);
        if (B_padding_buffer.size() < b_size) B_padding_buffer.resize(b_size, 0.0f);
        if (C_padding_buffer.size() < c_size) C_padding_buffer.resize(c_size, 0.0f);

        float* A_padding = A_padding_buffer.data();
        float* B_padding = B_padding_buffer.data();
        float* C_padding = C_padding_buffer.data();

        if (m_padding == 0) {
            memcpy(A_padding, A + thread_m_start * K, dim_m_per_thread * K * sizeof(float));
        } else {
        ...
        // 优化C的拷贝
        if (n_padding == 0 && m_padding == 0) {
            memcpy(C_padding, C + thread_m_start * N + thread_n_start,
                dim_m_per_thread * dim_n_per_thread * sizeof(float));
        } else {
            for (int m = thread_m_start; m < thread_m_end; m++) {

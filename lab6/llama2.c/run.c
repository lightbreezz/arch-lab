...
void build_transformer(Transformer *t, char* checkpoint_path) {
    ...
    extern void init_cuda_weights(float* h_weights_base, size_t bytes);
    float* weights_ptr = t->data + sizeof(Config)/sizeof(float);
    size_t weights_bytes = (size_t)t->file_size - sizeof(Config);
    init_cuda_weights(weights_ptr, weights_bytes);
}

void free_transformer(Transformer* t) {
    extern void free_cuda_weights();
    free_cuda_weights();
...
void matmul(float* xout, float* x, float* w, int n, int d) {
    ...
    if ((int64_t)d*n >= gpu_threshold) {
        // printf("Using GPU for matmul of size %d x %d\n", d, n);
